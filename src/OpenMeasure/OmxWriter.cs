...
/// <summary>
/// Streaming writer for OMX files. Supports incremental writing of measurement data.
/// </summary>
public sealed class OmxWriter : IDisposable
{
    ...
    internal OmxWriter(FileStream stream)
    {
        _stream = stream;
    ...
    public Dictionary<string, OmxValue> Properties => _fileProperties;
    ...
    {
        var groups = new List<OmxGroupDefinition>();
        int globalIndex = 0;

        foreach (var group in _groups)
        {
            var channelDefs = new List<OmxChannelDefinition>();
            ...
            groups.Add(new OmxGroupDefinition(
                group.Name,
                new Dictionary<string, OmxValue>(group.Properties),
                channelDefs));
        }
...
/// <summary>
/// Writer for a single group within an OMX file.
/// </summary>
public sealed class GroupWriter
{
    public string Name { get; }
    public Dictionary<string, OmxValue> Properties { get; } = [];
    internal List<ChannelWriter> ChannelWriters { get; } = [];
    private readonly int _groupIndex;
    private readonly OmxWriter _writer;

    internal GroupWriter(string name, int groupIndex, OmxWriter writer)
    {
        Name = name;
    ...
    {
        var dataType = OmxDataTypeExtensions.FromClrType<T>();
        ...
        where T : unmanaged
    {
        ...
        channel.Properties["omx.source_channel"] = sourceChannelName;
        if (startBit >= 0) channel.Properties["omx.start_bit"] = startBit;
        if (bitLength >= 0) channel.Properties["omx.bit_length"] = bitLength;
        if (factor != 1.0) channel.Properties["omx.factor"] = factor;
        if (offset != 0.0) channel.Properties["omx.offset"] = offset;
        return channel;
    }
}
...
public abstract class ChannelWriter
{
    ...
    protected ChannelWriter(string name, OmxDataType dataType)
    {
        Name = name;
        DataType = dataType;
    }
...
public sealed class RawChannelWriter : ChannelWriter
{
    ...
    internal RawChannelWriter(string name) : base(name, OmxDataType.Binary) { }
