...
public class CanBusTests : IDisposable
{
    ...
    private string TempFile(string name = "can.meas") => Path.Combine(_tempDir, name);
    ...
    public void WriteAndRead_CanBusGroup_WithSignalDefinitions()
    {
        ...
        using (var writer = MeasFile.CreateWriter(path))
        {
            ...
            var ts = MeasTimestamp.FromDateTimeOffset(start);
    ...
    {
        var path = TempFile("multi_bus.meas");
        var start = new DateTimeOffset(2026, 3, 13, 10, 0, 0, TimeSpan.Zero);
        var ts = MeasTimestamp.FromDateTimeOffset(start);
    ...
    {
        var path = TempFile("can_large.meas");
        var start = new DateTimeOffset(2026, 3, 13, 10, 0, 0, TimeSpan.Zero);
