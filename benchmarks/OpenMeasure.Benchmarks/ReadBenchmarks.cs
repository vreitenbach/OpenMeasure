...
public class ReadBenchmarks
{
    ...
    public void Setup()
    {
        ...
        _floatFile = Path.Combine(_tempDir, "float.omx");
        using (var writer = OmxFile.CreateWriter(_floatFile))
        {
        ...
        // Pre-create CAN file
        _canFile = Path.Combine(_tempDir, "can.omx");
        using (var writer = OmxFile.CreateWriter(_canFile))
        {
        ...
        _multiFile = Path.Combine(_tempDir, "multi.omx");
        using (var writer = OmxFile.CreateWriter(_multiFile))
        {
            var group = writer.AddGroup("Data");
    ...
    [Benchmark(Description = "OMX Read Float32 (ReadAll)")]
    public float[] ReadFloat32()
    {
        using var reader = OmxFile.OpenRead(_floatFile);
        return reader["Data"]["Signal"].ReadAll<float>();
    }

    [Benchmark(Description = "OMX Read Multi-Channel (4ch)")]
    public int ReadMultiChannel()
    {
        using var reader = OmxFile.OpenRead(_multiFile);
        var ts = reader["Data"]["Time"].ReadAll<OmxTimestamp>();
    ...
    [Benchmark(Description = "OMX Read Statistics Only (no data)")]
    ...
    [Benchmark(Description = "OMX Decode CAN Signal from Raw")]
    public double[] DecodeCanSignal()
    {
        using var reader = OmxFile.OpenRead(_canFile);
        return reader["CAN1"].DecodeSignal("RPM");
    }

    [Benchmark(Description = "OMX Read CAN Raw Frames")]
